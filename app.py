...
with st.sidebar:
    ...
    st.subheader("チャート設定")
    candles = st.slider("表示ローソク本数（最大90）", min_value=20, max_value=90, value=90, step=5)
    lookback_days = st.select_slider(
        "取得期間（長いほど指標が安定します）",
        options=[180, 270, 360, 420, 540, 720, 1080],
        value=420,
    )
...
# Data fetch (cached)
# -------------------------
price_dict = fetch_ohlcv(selected_tickers, period_days=int(lookback_days), interval="1d")
